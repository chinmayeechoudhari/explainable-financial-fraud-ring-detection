...
def make_batches(frame, store, batch_size, neighbor_k):
    ...
    while start < len(frame):
        ...
        for _, row in chunk.iterrows(): store.add_row(row, int(row["_timestamp"]))
        start = end
...
def run_epoch(model, frame, store, optimizer, criterion, batch_size, neighbor_k, device):
    ...
    for batch in make_batches(frame, store, batch_size, neighbor_k):
        ...
        logits, _ = model(batch.transaction_features.to(device), batch.sender_features.to(device), batch.sender_delta_seconds.to(device), batch.receiver_features.to(device), batch.receiver_delta_seconds.to(device))
        loss = criterion(logits, batch.labels.to(device)); loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0); optimizer.step(); losses.append(float(loss.item()))
...
def main() -> None:
    ...
    parser.add_argument("--train-positives", type=int, default=100); parser.add_argument("--train-negatives", type=int, default=2000)
    parser.add_argument("--eval-positives", type=int, default=100); parser.add_argument("--eval-negatives", type=int, default=2000)
    parser.add_argument("--epochs", type=int, default=5); parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--neighbor-k", type=int, default=10); parser.add_argument("--lr", type=float, default=1e-3); parser.add_argument("--seed", type=int, default=42)
    ...
    scaler = TemporalFeatureScaler.fit(train_source)
    train_source = scaler.transform_frame(train_source); val_source = scaler.transform_frame(val_source)
    ...
    model = TGATTransactionModel(transaction_dim=len(ALL_FEATURES), event_dim=len(ALL_FEATURES), hidden_dim=32, time_dim=16, num_heads=2, dropout=0.2).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4); criterion = torch.nn.BCEWithLogitsLoss()
    print(f"Device: {device}"); print(f"Source rows: train={len(train_source)}, validation={len(val_source)}")
    ...
    print("Feature scaling: train-only mean/std, clipped to [-10, 10]")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if pr > best_pr:
            best_pr, best_roc = pr, roc; best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
    ...
    print(f"Positive score mean: {pos_scores.mean():.6f}"); print(f"Negative score mean: {neg_scores.mean():.6f}")
    print(f"Sanity runtime: {time.time() - start_time:.1f}s"); print("TGAT LEARNING SANITY TEST: COMPLETED")
